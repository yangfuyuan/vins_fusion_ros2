...
#if 1
      jacobian_feature = reduce * ric2.transpose() * Rj.transpose() * Ri * ric *
...
void ProjectionTwoFrameTwoCamFactor::check(double **parameters) {
  ...
  std::cout << Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>>(jaco[2])
            << std::endl
            << std::endl;
  std::cout << Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>>(jaco[3])
            << std::endl
            << std::endl;
  std::cout << Eigen::Map<Eigen::Vector2d>(jaco[4]) << std::endl << std::endl;
  std::cout << Eigen::Map<Eigen::Vector2d>(jaco[5]) << std::endl << std::endl;
  ...
  for (int k = 0; k < 26; k++) {
    ...
    Eigen::Vector3d tic2(parameters[3][0], parameters[3][1], parameters[3][2]);
    Eigen::Quaterniond qic2(parameters[3][6], parameters[3][3],
                            parameters[3][4], parameters[3][5]);
