...
class InitialPoseFactor : public ceres::SizedCostFunction<6, 7> {
 public:
    ...
    if (jacobians) {
      if (jacobians[0]) {
        ...
        jacobian_pose.block<3, 3>(3, 3) =
            Utility::Qleft(init_Q.inverse() * Q).bottomRightCorner<3, 3>();
  ...
  void check(double **parameters) {
    ...
    std::cout << Eigen::Map<Eigen::Matrix<double, 6, 1>>(res).transpose()
              << std::endl
              << std::endl;
    std::cout << Eigen::Map<Eigen::Matrix<double, 6, 7, Eigen::RowMajor>>(
                     jaco[0])
              << std::endl
              << std::endl;
